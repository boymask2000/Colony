...
public abstract class Coltivatore extends EdificioFabbrica implements NotifyFromAgent {
	...
	private List<CosaCheCresce> piante = new ArrayList<CosaCheCresce>();
	...
	public void work() {
		...
		a.goMission(v.x, v.y, new Mission() {
			...
			@Override
			public void exec() {
				CosaCheCresce c = createItem(v.x, v.y);
				piante.add(c);
	...
	private void faiRaccolto() {
		if(piante.size()==0)return;
		if (indexRaccolto >= piante.size())
			indexRaccolto = 0;
		if( piante.get(indexRaccolto).isAdult()) {
			numArticoliProdotti++;
			 piante.get(indexRaccolto).resetCrescita();
		}
