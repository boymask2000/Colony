	...
	public ArrayList<MapObject> getTileList(String propertyName) {
		...
		for (MapLayer layer : tiledMap.getLayers()) {
			for (MapObject obj : layer.getObjects()) {
				...
				Iterator<String> propertyKeys = defaultProps.getKeys(); // iterate over keys; copy default values into
																		// props if needed

				while (propertyKeys.hasNext()) {
					String key = propertyKeys.next(); // check if value already exists; if not, create property with
														// default value
